...
def print_tabla(tabla, alfabeto, estados):
    ...
    for i in range(0, len(alfabeto)):
        if i+1==len(alfabeto):
          print("["," "*math.ceil((aux[i]-3)/2),alfabeto[i]," "*math.trunc((aux[i]-3)/2),"]",sep="", end="");  
        else:    
          print("["," "*math.ceil((aux[i]-3)/2),alfabeto[i]," "*math.trunc((aux[i]-3)/2),"] | ",sep="", end="");
    print("");
...
def load_file(ruta):
    ...
    for line in file:
      ...
      if i == 1:
        ...
        # print(line);
        estados = line.split(" "); del estados[0]; j = 0;
        ...
        estadosfinales = line.split(" "); del estadosfinales[0]; j = 0;
        ...
        print(" + Esto nunca debería de aparecer [1]"); print(line.replace("\n", ""));
      # print("</Línea ",i," >");
    #print("</Tabla de transiciones >");
