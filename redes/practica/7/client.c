...
int main(int argc, char const *argv[]) {
    ...
    propia.sin_port = htons(puerto + 1);      // Número de puerto (de orden de host a orden de red)
    ...
    if ((bytes = recvfrom(socketFD, mensaje, MAX_STRING, 0, (struct sockaddr*)&servidor, NULL)) < 0) {
        perror("recvfrom");
        exit(EXIT_FAILURE);
    }
